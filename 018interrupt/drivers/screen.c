...
void scroll(unsigned char *vidmem);
...
void scroll(unsigned char *vidmem){
    ...
    for (i = 1 * max_cols_2; i < MAX_ROWS * max_cols_2; i++) {
        vidmem[i-max_cols_2] = vidmem[i];
    }

    /* Blank last line */
    char *last_line = (MAX_ROWS - 1) * max_cols_2 + vidmem;
...
int print_char(char c, int col, int row, char attr) {
    ...
    // attr default "white on black"
    if (!attr){
        attr = WHITE_ON_BLACK;
    }

    // if index is out of the range. print red char "e" at the end. 
    ...
    int offset;
    // if 'col' and 'row' are negative, offset is the postion of the cursor.
    if (col >= 0 && row >= 0){
        offset = get_offset(col, row);
    }
    else {
    ...
    // if print new line, just set the offset to next line head.
    if (c == '\n') {
        row = get_offset_row(offset);
